...
(function($) {
    var cache = [];
    $.fn["swipePrevNext"] = function(opts) {
        ...
        for (var i = 0; i < this.length; i++) {
            tmp = new swipePrevNext(this[i], opts);
            if (this[i].id)
                cache[this[i].id] = tmp;
        }
        return this.length == 1 ? tmp : this;
    };
    var swipePrevNext = (function() {
        var swipePrevNext = function(elID, opts) {
            var that = this;
